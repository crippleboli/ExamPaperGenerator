...
def _get_difficulty_criteria(subject, question_type, difficulty):
    criteria = {
        "高等数学": {
            "选择题": {
                "简单": "基础概念直接应用（如极限、导数定义）",
                "中等": "需结合2-3个知识点（如导数应用+函数图像）",
                "困难": "涉及抽象概念或复杂计算（如多元函数极值、级数判敛）"
            },
            "计算题": {
                "简单": "单一知识点计算（如求导、积分）",
                "中等": "跨章节综合运算（如微分方程+积分应用）",
                "困难": "多步骤证明或复杂推导（如泰勒展开应用、曲面积分）"
            },
            "填空题": {
                "简单": "直接公式填空（如导数公式、积分结果）",
                "中等": "需初步计算的填空题（如极限值、切线方程）",
                "困难": "涉及综合分析的填空（如收敛域、参数方程转换）"
            }
        },
        "线性代数": {
            "选择题": {
    ...
    return criteria.get(subject, {}).get(question_type, {}).get(difficulty, "")
# =====================================================================================
